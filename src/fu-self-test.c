...
fu_plugin_hwdb_func (void)
{
	...
	ret = fu_hwdb_load (hwdb, &error);
	g_assert_no_error (error);
	g_assert (ret);
	fu_plugin_set_hwdb (plugin, hwdb);

	tmp = fu_plugin_lookup_hwdb_by_id (plugin, "fwupd-Tests", "USB\\VID_0A5C&PID_6412");
	g_assert_cmpstr (tmp, ==, "ignore-runtime");
	tmp = fu_plugin_lookup_hwdb_by_id (plugin, "fwupd-Tests", "USB\\VID_FFFF&PID_FFFF");
	g_assert_cmpstr (tmp, ==, "");
	tmp = fu_plugin_lookup_hwdb_by_id (plugin, "fwupd-Unfound", "baz");
	g_assert_cmpstr (tmp, ==, NULL);
	tmp = fu_plugin_lookup_hwdb_by_id (plugin, "fwupd-tests", "unfound");
	g_assert_cmpstr (tmp, ==, NULL);
	tmp = fu_plugin_lookup_hwdb_by_id (plugin, "fwupd-unfound", "unfound");
	g_assert_cmpstr (tmp, ==, NULL);
}
...
main (int argc, char **argv)
{
	...
	g_test_add_func ("/fwupd/plugin{hwdb}", fu_plugin_hwdb_func);
