				 ...
				 GError **error)
{
	...
	if (g_bytes_get_size (blob) > dfu_sector_get_size (sector)) {
		//FIXME: dfu_element_get_address must play a role?
		g_error ("FIXME");
	}

	/* chunk up the memory space into pages */
	packets = dfu_chunked_new (g_bytes_get_data (blob, NULL),
			       ...
			       GError **error)
{
	...
	/* get default sector so we know the firmware size */
	sector = dfu_target_get_sector_default (target);
	if (sector == NULL) {
		g_set_error_literal (error,
				     FWUPD_ERROR,
	...
	/* chunk up the memory space into pages */
	packets = dfu_chunked_new (NULL,
				   dfu_sector_get_size (sector) - address,
				   dfu_sector_get_address (sector) + address,
				   ATMEL_64KB_PAGE,
				   ATMEL_MAX_TRANSFER_SIZE);
